...
def train_model(
    ...
    for epoch in range(0, max_iter[0]):
        # Logger
        print('Stage 1 Epoch {}/{}'.format(epoch + 1, max_iter[0]))
        print('-' * 20)
        with open(os.path.join(result_log_dir, fileindex + '.txt'), mode='a+', encoding='utf-8') as f:
            f.write('Epoch {}/{}\n'.format(epoch + 1, max_iter[0]))
            f.write('-' * 20)
            f.write('\n')
            f.close()
    ...
    print('Stage 1 training complete in {:.0f}m {:.0f}s'.format(time_elapsed // 60, time_elapsed % 60))
